...
任何情况下，当星舰的两个主要部分必须进行分离并执行各自的不同飞行任务时，分离飞行模式便会启动。在常规情况下，分离飞行模式使用的是巡航模式的变体（详见 <a href="/docs/tng-technical-manual/uss-enterprise-flight-operations.html#_15-4-巡航模式-cruise-mode" class="cus-link">条目 15.4 <i class="fa-solid fa-arrow-right-from-bracket"></i></a> ），而在紧急情况下则会使用红色警戒模式的变体（详见 <a href="/docs/tng-technical-manual/uss-enterprise-flight-operations.html#_15-6-红色警戒模式-red-alert" class="cus-link">条目 15.6 <i class="fa-solid fa-arrow-right-from-bracket"></i></a> ）。常规情况下的舰碟分离主要在大修期间或性能验收试飞与操纵性检查期间进行，此时分离所造成的风险基本可以忽略。分离模式的操作规程包括：
- 在每班开始时在所有的主系统和战术系统上运行 4 级自动诊断序列（根据特定的运作和安全规程，关键系统可能需求更为频繁的诊断）；
